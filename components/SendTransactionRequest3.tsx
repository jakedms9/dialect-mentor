...
export const SendTransactionRequestSe: FC<SendTransactionRequestProps> = ({ reference }) => {
  ...
  const onClick = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`/api/send3?network=mainnet-beta&reference=${reference.toBase58()}`, {
        account: publicKey
      }, {

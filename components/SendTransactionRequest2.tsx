...
export const SendTransactionRequestSo: FC<SendTransactionRequestProps> = ({ reference }) => {
  ...
  const onClick = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.post(`/api/send2?network=mainnet-beta&reference=${reference.toBase58()}`, {
        account: publicKey
      }, {
